...
public class ColorPixel implements Pixel {
	...
	public Pixel blend(Pixel p, double weight) {
		if(weight > 1.0 || weight < 0.0) {
			throw new RuntimeException("Not valid number");
		}
		double redAvg = (p.getRed()*(1 - weight) + this.getRed()*weight);
		double blueAvg = (p.getBlue()*(1 - weight) + this.getBlue()*weight);
		double greenAvg = (p.getGreen()*(1 - weight) + this.getGreen()*weight);
		
		return new ColorPixel(redAvg, greenAvg, blueAvg);
	}
}
